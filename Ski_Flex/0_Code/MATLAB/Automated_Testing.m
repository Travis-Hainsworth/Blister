...
global arduino_port inclinometer_port force_gage1_port force_gage2_port; %inclinometer_port_back
global arduino_serial inclinometer_serial force_gage1_serial force_gage2_serial; %inclinometer_back_serial
global ei_dtheta ei_moment ei_displacment gj_dtheta gj_moment gj_displacment ei_distance_from_tip;

% Serial USB connections
arduino_port = 'COM16';                       % write in arduino port
inclinometer_port = 'COM12';            % write in front inclometer port
...
arduino_serial = serialport(arduino_port, 115200);
...
while num_of_test ~= 0
    ...
    sig = return_to_start(arduino_serial);
    %%
    sig = move_force_gauges(arduino_serial, 100, 100);
%%
    pause(1);

    
    %%%% HAVE TO MOVE THE INCLINOMETER TO THE MIDDLE TO LEVEL %%%
    sig = level_force_gauges(arduino_serial, 0, .05, inclinometer_serial, 1);
    ...
    sig = return_to_start(arduino_serial);
    %%
    sig = move_force_gauges(arduino_serial, 0, -20);
    %%
    ...
    %move_force_gauges_until_desired_force(arduino_serial, force_gage1_serial, force_gage2_serial, 0, 5);
...
sig = move_force_gauges(arduino_serial, 0, -1);
disp(sig);
%% Manually move inclinometer motor
sig = move_x_mm(100,1, arduino_serial);
disp(sig);
...
function data_matrix = sensor_automation(test_interval_mm, direction) %inclinometer_serial
    global arduino_serial inclinometer_serial force_gage1_serial force_gage2_serial;
    ...
    while stop_num~=42
        ...
        %MOVE SENSOR
        sig = move_x_mm(-test_interval_mm, direction, arduino_serial);
        disp(sig);
