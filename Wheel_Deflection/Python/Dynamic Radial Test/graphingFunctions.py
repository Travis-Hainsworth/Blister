...
def comparison_plot(file_paths, do_you_want_lateral):
    counter = 0
    fig = go.Figure()
    colors = ['Red', 'Blue', 'Green', 'Yellow', 'Purple', 'Black', 'Gray', 'Pink', 'Orange']
    ...
    for file_path in file_paths:
        ...
        fig.add_trace(go.Scatter(x=sorted_height, y=regression_curve, mode='lines', line=dict(color=colors[counter]),
                                 name=f'Regression Curve Radial {rim} {head} head<br>r2 = {r2: .4f}'))

        if do_you_want_lateral == 'yes':
            mocap_lat, max_defs_lat, _, height_lat, _, _ = dataProcessingMain(file_path, axis='z')

            max_def_inches_lat = [i * .0393701 for i in max_defs_lat]

            height_lat = np.array(height_lat).astype(float)
            ...
            fig.add_trace(go.Scatter(x=sorted_height_lat, y=regression_curve_lat,
...
def comparison_plot_mean(file_paths):
    ...
    for file_path in file_paths:
        ...
        std_values = [np.std(sorted_max_def_inches[sorted_height == value]) for value in np.unique(sorted_height)]

        regression_coefficients = np.polyfit(np.unique(sorted_height), mean_values, 2)
