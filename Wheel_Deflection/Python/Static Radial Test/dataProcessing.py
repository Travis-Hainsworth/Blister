...
# Settling time function implementation
def settling_time(x, d):
    # Calculate the mid-reference level
    mid_ref = (max(x) + min(x)) / 2

    # Calculate the upper and lower state boundaries
    upper_boundary = max(x) - 0.02 * (max(x) - min(x))
    lower_boundary = min(x) + 0.02 * (max(x) - min(x))

    # Find the transitions
    transitions = np.where((x[:-1] <= lower_boundary) & (x[1:] >= upper_boundary))[0]

    # Find the first transition that occurs within the duration d
    for transition in transitions:
        if x[transition + 1] - x[transition] >= d:
            return transition + 1

    # If no settling time is found, return NaN
    return np.nan
